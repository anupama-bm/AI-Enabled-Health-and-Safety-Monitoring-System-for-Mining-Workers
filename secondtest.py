...
BLINK_MIN_FRAMES        = 2
BLINK_MAX_FRAMES        = 12
BLINK_MIN_SEPARATION_MS = 250         # ms between blinks
    ...
    def update(self, ear_l: float, ear_r: float, ts: float) -> bool:
        ...
        if is_closed:
            ...
            if self._in_blink:
                ...
                gap_ok  = (ts - self._last_blink_ts) * 1000 >= BLINK_MIN_SEPARATION_MS
                dur_ok  = BLINK_MIN_FRAMES <= self.closed_frames <= BLINK_MAX_FRAMES
                if dur_ok and gap_ok:
                    self.count += 1
                    self._last_blink_ts = ts
                    blink_detected = True
                    print(f"[BLINK] #{self.count}  dur={self.closed_frames}f  "
